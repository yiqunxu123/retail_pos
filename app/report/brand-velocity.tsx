...
export default function BrandVelocityReportScreen() {
  ...
  const columns: ColumnDefinition<BrandReportView>[] = [
    {
      ...
      hideable: false,
      render: (item) => (
        <Text className="text-gray-800 font-medium pr-2" numberOfLines={2}>
          {item.brandName}
        </Text>
      ),
    },
    {
      ...
      align: "center",
      visible: true,
      render: (item) => <Text className="text-gray-800">{item.qtySold}</Text>,
    },
    {
      key: "salesRevenue",
      title: "Sales Revenue ($)",
  ...
  const handleSearch = (item: BrandReportView, query: string) => {
    return item.brandName.toLowerCase().includes(query.toLowerCase());
  };

  const handleFilter = (item: BrandReportView, filters: Record<string, string | null>) => {
  ...
  const handleSort = (data: BrandReportView[], sortBy: string | null) => {
  ...
  return (
    <View className="flex-1 bg-gray-50">
      <PageHeader title="Brand Velocity Report">
